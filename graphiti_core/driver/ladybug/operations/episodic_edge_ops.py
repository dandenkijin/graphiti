...
from graphiti_core.driver.record_parsers import episodic_edge_from_record
...
class LadybugEpisodicEdgeOperations(EpisodicEdgeOperations):
    async def save(
        ...
        # Use a basic save query for LadybugDB episodic edges
        query = """
    ...
    async def get_by_uuid(
        ...
        query = f"""
        MATCH (source:Episodic)-[r:HAS_EPISODE]->(target:Episodic)
        WHERE r.uuid = $uuid
        RETURN {EPISODIC_EDGE_RETURN}
        """
        params = {'uuid': uuid}
        result = await executor.execute_query(query, params, tx)
        
        if not result:
            raise EdgeNotFoundError(f'Episodic edge with uuid {uuid} not found')
        
        return episodic_edge_from_record(result[0])
    ...
    async def get_by_source_target(
        ...
        return [episodic_edge_from_record(record) for record in result]
